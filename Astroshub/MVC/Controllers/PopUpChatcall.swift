...
class PopUpChatcall: UIViewController, WWCalendarTimeSelectorProtocol {
    ...
    {
        if Validate.shared.validatepop(vc: self)
        {
    ...
    func CallstatusMethods() {
        
        let deviceID = UIDevice.current.identifierForVendor!.uuidString
        print(deviceID)
        let setparameters = ["app_type":MethodName.APPTYPE.rawValue,"app_version":MethodName.APPVERSION.rawValue,"astrologer_id":user_id,"astrologer_api_key":user_apikey,"datetime":self.postdob,"status":is_ONOFF]
        print(setparameters)
        //ActivityIndicator.shared.startLoading()
        AutoBcmLoadingView.show("Loading......")
        AppHelperModel.requestPOSTURL(MethodName.UPDATECALLSTATUS.rawValue, params: setparameters as [String : AnyObject],headers: nil,
                                      success: { (respose) in
